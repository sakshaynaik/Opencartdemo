...
public class TC115_OIreorderingfromoitest extends BaseClass{

	@Test(dataProvider = "validcredentials")
	public void orderInformationReorderProduct(String validdata) {

		log.info("***** TC115_OIreorderingfromoitest Started *****");
		...
		orderinfopg.clickOnReorderButton();
		Assert.assertTrue(orderinfopg.isDisplayedReorderSuccessMsg());
		log.info("ReOrder Success Message: " + orderinfopg.getReorderSuccessMsg());
			
		Shoppingcartpage shopcartpg = orderinfopg.clickOnShoppingCartLink();
		Assert.assertEquals(shopcartpg.getShoppingCartPageTitle(), config.getShoppingCartPageTitle());
		log.info("Shopping Cart Page Title: " + shopcartpg.getShoppingCartPageTitle());
		
		Assert.assertTrue(shopcartpg.isDisplayedOrderedProductImage());
		log.info("ReOrdered Product Displayed On webpage");

		log.info("***** TC115_OIreorderingfromoitest Completed *****");
	}
