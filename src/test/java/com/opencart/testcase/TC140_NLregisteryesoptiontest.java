...
public class TC140_NLregisteryesoptiontest extends BaseClass{
	...
	public void registeringWithNewsLetter(HashMap<String, String> hMap) {
		...
		Assert.assertFalse(nwspg.isSelectedNewsletterRadioOption());
		log.info("NO Option Selected By Default In Newsletter Page");

		nwspg.clickOnContinueButton();
		Assert.assertTrue(accpg.isDisplayedNewsLetterSuccessMsg());
		log.info("NewsLetter Success Message: " + accpg.getNewsLetterSuccessMsg());

		accpg.clickOnNewsLetterLink();
		Assert.assertEquals(nwspg.getNewsletterPageTitle(), config.getNewsletterPageTitle());
		log.info("NewsLetter Page Title: " + nwspg.getNewsletterPageTitle());

		log.info("***** TC140_NLregisteryesoptiontest Completed *****");
